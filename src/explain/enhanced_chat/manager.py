...
class EnhancedChatManager:
    ...
    async def check_job_status(self, job_tag: str | None = None) -> dict[str, Any]:
        """Check the status of a job"""
        if job_tag is None:
            job_tag = self.active_jobs["latest"]["tag"]

        try:
            status_response = await bulk_controller_v1_status.asyncio(client=self.client, body=[job_tag])
                ...
                if not job_in_local and completed > 0:
                    ...
                    try:
                        ...
                        return {
                            ...
                            "success_rate": completed / (completed + failed + in_progress)
                            if (completed + failed + in_progress) > 0
                            else 0,
    ...
    async def wait_for_completion(
        self, job_tag: str, poll_interval: int = 3, timeout: int | None = None, show_progress: bool = True
    ) -> bool:
    ...
    async def collect_results(
        self, job_tag: str, wait_for_completion: bool = True, timeout: int | None = None, refetch: bool = False
    ) -> JobResult:
        ...
        try:
            ...
            if wait_for_completion:
                ...
                if not completed:
                    ...
                    logger.warning(
                        f"Job {job_tag} did not complete within timeout, attempting to collect partial results"
                    )
            ...
            if not job_in_active and status.get("server_completed", False):
                ...
                try:
                    ...
                    if status_response and status_response.done:
                        ...
                        export_response = await bulk_controller_v1_export.asyncio(client=self.client, body=ref_ids)
            ...
            export_response = await bulk_controller_v1_export.asyncio(
                client=self.client, body=[ref.id for ref in created_response.refs]
            )
    ...
    async def submit_and_collect(
        self,
        prompts: TaggedPrompt | list[TaggedPrompt],
        job_tag: str,
        config: ChatConfiguration | None = None,
            ...
            try:
                ...
                for message in conversation.messages:
                    message_data = {
                        ...
                        "spans": [
                            {"begin": span.begin, "end": span.end, "id": span.id, "type": str(span.type_)}
