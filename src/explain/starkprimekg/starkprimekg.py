...
_DEFAULT_KG_DIR = os.environ.get('STARK_PRIMEKG_DIR', '')
_DEFAULT_DRUGBANK_XML = os.environ.get('DRUGBANK_XML_PATH', '')

class StarkPrimeKG:
    def __init__(self, save_address=_DEFAULT_KG_DIR,
                 enc_language_model_name='pritamdeka/S-PubMedBERT-MS-MARCO',
                 node_emb_save_address=_DEFAULT_KG_DIR,
                 drugbank_xml_path: str = _DEFAULT_DRUGBANK_XML):
        """
        ...
        # Load node info
        with open(os.path.join(save_address, 'node_info.json')) as f:
    ...
    def text_to_embedding(self, texts: str | list[str]) -> torch.Tensor:
